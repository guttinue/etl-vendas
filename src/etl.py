...
def tratarDados(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Order Date"] = pd.to_datetime(df["Order Date"], format='%m/%d/%Y %H:%M', errors='coerce') #especificando o formato da data
...
def main():
    ...
    salvarDataFrame(df_tratado, Path("../etl-vendas/data/vendas_unificadas.csv"))
